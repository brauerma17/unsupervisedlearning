...
writeFn = fn[:-4] + "_ica_EM_output.csv"
print(writeFn)
...
def runEverything():
    ...
    results = []
    for n_comp in range(1,len(x[0])+1):
        for whiten in whitens:
            for norm in norms:
                for alg in algs:
                    for cov in covs:
                        err,kur0,kur1,log = classify(x,y,normx,normy,n_comp,whiten,norm,alg,cov)
                        print(n_comp,whiten,norm,alg,cov,":",err,kur0,kur1,log)
                        results.append([n_comp,whiten,norm,alg,cov,err,kur0,kur1,log])

    file = open(writeFn,"w",newline="")
    csvW = csv.writer(file)
    csvW.writerow(["n_comp","whiten","norm","alg","cov","error","kur0","kur1","log"])
...
def classify(x,y,normx,normy,n_comp,whiten,norm,alg,cov):
    if norm:
        x = normx
        y = normy
    ...
    return (min(sum(abs(fitter.predict(x)-y)),sum(abs((1 - fitter.predict(x)) - y)))/len(x),kur0,kur1,fitter.lower_bound_)
