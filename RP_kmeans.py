...
writeFn = fn[:-4] + "_rp_kmeans_output.csv"
print(writeFn)
...
def runEverything():
    ...
    for n_comp in range(1,len(x[0])+1):
        for norm in norms:
            for i in range(0,3):
                err = classify(x,y,normx,normy,n_comp,norm)
                print(n_comp,norm,":",err)
                results.append([n_comp,norm,err])

    file = open(writeFn,"w",newline="")
    csvW = csv.writer(file)
    csvW.writerow(["n_comp","norm","error"])
...
def classify(x,y,normx,normy,n_comp,norm):
    if norm:
        x = normx
        y = normy
